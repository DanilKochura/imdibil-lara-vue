...
{{--                <img class="border border-4 border-success rounded-circle shadow-warning-xlg-hover user-avatar" src="{{asset('images/uploads/'.($user->avatar ?: 'default.jpg'))}}">--}}
            ...
            <div class="col-sm-4 col-6 ">
                <div class="d-flex flex-row align-items-center justify-content-evenly">
                    <h1 class="text-gold text-center">{{$user->name}}</h1>
                    {{--                    <span class="badge badge-success badge-soft ms-1">{{$user->getRole()}}</span>--}}
                </div>
                @if($user->isExpert())
                    <div class="d-flex justify-content-evenly">
                        <div>
                            <div class="rate-ch">{{round($user->rates->pluck('rate')->avg(), 2)}}</div>
                            <div class="text-secondary fs-10p">Ср. оценка</div>
                        </div>
                        <div>
                            <div class="text-white">{{$user->rates->pluck('rate')->count()}}/{{$meets_count}}</div>
                            <div class="text-secondary fs-10p">Кол-во встреч</div>
                        </div>
                    </div>
                @else
                ...
                <div class="d-flex">
                    @foreach($medals as $rank => $medal)
                        ...
                        <div type="button" class="btn position-relative">
                            <x-medals type="medal"
                                      class="{{ ($rank == 3) ? 'fill-warning' : (($rank == 2) ? 'fill-gray-600' : 'fill-brown') }} avatar">

                            </x-medals>
                            <span
                                class="position-absolute medal-counter translate-middle badge rounded-pill bg-success">
                                {{count($medal)}}
	                    </span>
                        </div>
            ...
            <div class="col-sm-4 text-black">
                ...
                <div
                    ...
                    @if(auth()->id() == $user->id)
                        ...
                        @if($user->isExpert())
                            <button type="button" class="btn btn-warning rounded-circle ms-2 mb-2"
                                    data-bs-toggle="modal"
...
{{--                            </button>--}}
        ...
        @if($quiz->count())
            <div class=" d-block d-md-none bg-white"  style="border-radius: 30px 30px 0 0">
                <p class="h4 text-center">Прогресс викторины</p>
                @foreach($quiz as $progress)
                    <div class="col-12  mt-2">
                        <div class="row align-items-center">
                            <div class="text-center col-4">
                                <a href="{{route('quiz', $progress->quiz->alias)}}" class="link-dashed-gold link-dark">
                                    {{$progress->quiz->title}}
                                </a>
                            </div>
                                <div class="col-2">
                            ...
                            <div class="col-2">
                                ...
                                <div class="align-items-center d-flex justify-content-evenly">
                                    ...
                                    @if($progress->quiz->questionsCount() == $progress->points)
                                        <x-medals type="{{$progress->quiz->medal}}" class="fill-super avatar"
                                                  data-bs-toggle="tooltip"
                                                  data-bs-placement="top" title="Пройдено 100%"></x-medals>
                                    @elseif($rank == 3)
                                        <x-medals type="{{$progress->quiz->medal}}"
                                                  class=" {{($rank and $rank == 3) ? '' : 'fill-inactive'}} fill-warning avatar"
                                                  data-bs-toggle="tooltip"
                                    ...
                                    <p class="text-center">
                                        @if($progress->date_cert)
                                            <a target="_blank" class="link-secondary"
                                               href="{{asset('/certs/'.$progress->id.'.pdf')}}">
...
{{--                                    <img src="{{asset('images/posters/'.$movie->poster)}}" class="img-fluid" alt="...">--}}
            ...
            <div class="row bg-white">
                ...
                <div class="col-12">
                    @foreach($user->rates as $rate)
                        <div class="row m-2 rounded-3 shadow align-items-center">
                            <div class="col-2 mx-0 px-0">
                                <img class="img-fluid" style="border-radius: .45rem 0 0 .45rem !important" src="{{asset('images/posters/'.$rate->meeting->movie->poster)}}">
                            </div>
                            <div class="col-7">
                                <a class="fs-4 link-dark link-dashed-gold mov-nam"
                                   href="{{$rate->meeting->movie->url}}">{{$rate->meeting->movie->name_m}}</a>
                            </div>
                            <div class="col-1">
                                <div class="rate-ch fs-2 mb-0">
                                    {{$rate->rate}}
                                </div>
                            </div>
                            <div class="col-1">
                                <div class="text-secondary fst-italic mb-0">
                                    ({{$rate->meeting->movie->our_rate}})
                                </div>
                            </div>
                        </div>
                        ...
                        @foreach($quiz as $progress)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="d-flex justify-content-evenly   ">
                                        <x-medals type="{{$progress->quiz->medal}}"
                                                  class=" {{($rank and $rank > 0) ? '' : 'fill-inactive'}} fill-brown avatar"
                                                  data-bs-toggle="tooltip"
                                                  data-bs-placement="top" title="Пройдено 60%"></x-medals>
                                        <x-medals type="{{$progress->quiz->medal}}"
                                                  class=" {{($rank and $rank > 1) ? '' : 'fill-inactive'}} fill-gray-200 avatar"
                                                  data-bs-toggle="tooltip"
                                                  data-bs-placement="top" title="Пройдено 80%"></x-medals>
                                        @if($progress->quiz->questionsCount() == $progress->points)
                                            <x-medals type="{{$progress->quiz->medal}}" class="fill-super avatar"
    ...
    <div class="modal fade" id="pairAddModal" tabindex="-1" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    transform: translate(0px, 53px);">
                    ...
                    @else
                        <div class="row text-center">
                            ...
                            <div class="col-6"><img
                                    src="{{asset('/images/posters/'.$user->pair[0]->secondMovie->poster)}}"
                                    class="img-fluid rounded" style="width: 150px; height: 200px" alt=""></div>
                            <div class="col-6"><img
                                    src="{{asset('/images/posters/'.$user->pair[0]->firstMovie->poster)}}"
                                    class="img-fluid rounded" style="width: 150px; height: 200px" alt=""></div>
                            <form action="{{route('profile.delete-pair', $user->pair[0])}}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-warning mt-3" onclick="delete_user_pair({{$user->id}})">
                                    Удалить
                                </button>
                            </form>
                        </div>
